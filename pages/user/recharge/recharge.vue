	...
	<view class="bodyMain">
		...
		<view class="main">
			<view class="balance flex-space-between m-radius">
				<text>保证金余额：</text>
				<text class="money">{{ storeInfo.balance ? formatPrice(storeInfo.balance) : "$0.00" }}</text>
				<view class="foot flex-space-between">
					<text class="info">商家可以提交提现申请</text>
					<text class="btn flex-center m-radius" @click="navigateTo">余额提现</text>
				</view>
			</view>
		</view>
				...
				<swiper-item v-for="(item, index) in list" :key="item.id">
					<view class="swiperItem padding m-radius flex-center">
						...
						<view class="cardInfo">
							...
							<text class="title text flex-center">保证金充值</text>
...
export default {
	...
	methods: {
		getList() {
			this.$http.get(this.$api.getAmounts, { type: 1 }).then(res => {
				this.list = res.list;
			});
		},
		swiperChange(e) {
		...
		isPay() {
			if(this.list && this.list.length > this.current) {				
				uni.navigateTo({
					url: "../pay/pay?payMoney=" + this.list[this.current].amount
				});
			} else {
				uni.showToast({
					title: "请选择充值金额",
