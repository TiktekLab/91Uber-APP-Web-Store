import { mergeConfig, dispatchRequest} from "./utils.js";
export default class request {
	constructor(options) {
		...
		this.config = options.config || {
			isPrompt: true,  //是否弹框显示错误消息，如果为False，需自行在catch中处理
			load: false,
			isFactory: true,
			tokenRequired : true,
            resend: 0
	...
	async request(data) {
		...
		try {
			if (!data.url) {
				throw { errMsg: "Request缺失数据url", statusCode: 0}
			}
			// 数据合并
			...
			let requestResult = await dispatchRequest(requestInfo);
